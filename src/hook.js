...
function checkFieldRole(callerRole, field) {
    if (!field || !field.requiredRole)
        return null;
    if (!callerRole)
        return null; // can't enforce without a role hint
 ...
 */
async function assertMinRole(params, minRole, opts = {}) {
    let caller = null;
    let resolutionError = false;
    try {
        caller = await store.resolveCaller(params);
    }
    catch (_) {
...
async function assertValidCaller(params) {
    ...
    let email = null;
    try {
        email = await store.fetchCallerEmail(params);
    }
    catch (_) {
        email = null;
    }
    if (!email)
        return 'A valid Adobe IMS token is required for this action.';
    return null;
}
