...
function pickFirstHeader(headers, names) {
    ...
    for (const n of names) {
        if (headers[n])
            return String(headers[n]).replace(/^Bearer\s+/i, '').trim();
    }
    return null;
}
...
function groupsFromTokenClaims(token) {
    ...
    for (const r of roles) {
        if (!r)
            continue;
        if (r['named-role'])
            out.add(String(r['named-role']));
        if (r.role)
            out.add(String(r.role));
        if (r.principal && r.organization)
            out.add(`${r.organization}:${r['named-role'] || r.role || ''}`);
    }
    ...
    for (const p of ppc) {
        ...
        if (!ctx)
            continue;
        if (ctx.label)
            out.add(String(ctx.label));
        if (ctx.serviceCode)
            out.add(String(ctx.serviceCode));
        if (ctx.groupid != null)
            out.add('group:' + ctx.groupid);
    }
    ...
    try {
        ...
        for (const org of (Array.isArray(orgs) ? orgs : [])) {
            for (const g of (org.groups || [])) {
                if (g && g.groupName)
                    groups.push(String(g.groupName));
            }
        }
        return groups;
    }
...
async function main(params) {
    ...
    try {
        const profile = await fetchProfile(token, params.OAUTH_CLIENT_ID);
